...
bot.on("message", async (message) => { // eslint-disable-line
    ...
    if(command == "ping"){
        message.channel.send(`ping lu **${bot.ws.ping}ms**`);
      }else if(command === "stats"){
        message.channel.send(`ID: ${bot.user.id}
        ${bot.guilds.cache.size} guild(s)
...
> \`nowplaying\`, \`queue\`, \`volume\``)
            .setFooter("created by raffi baihaqy");
        message.channel.send(helpembed);
    }
    if (command === "play" || command === "p") {
        const voiceChannel = message.member.voice.channel;
        if (!voiceChannel) return message.channel.send("masuk voice channel dulu bgst !!!");
    ...
    if (command === "search" || command === "sc") {
        const voiceChannel = message.member.voice.channel;
        if (!voiceChannel) return message.channel.send("masuk voice channel dlu bgst !!!");
        const permissions = voiceChannel.permissionsFor(message.client.user);
        if (!permissions.has("CONNECT")) {
            return message.channel.send("Sorry, but I need a **`CONNECT`** permission to proceed!");
        }
        ...
        if (!message.member.voice.channel) return message.channel.send("masuk voice channel woi, baru bisa skip gblk !!!  ");
        if (!serverQueue) return message.channel.send("gada lagu yg diskip woi!!!");
        ...
        if (!message.member.voice.channel) return message.channel.send("masuk voice channel dlu bgst !!!");
        if (!serverQueue) return message.channel.send("gada lagunya gimana amo distop malih!!!");
        serverQueue.songs = [];
        ...
        if (!serverQueue) return message.channel.send("gada lagu yg mulai");
        ...
        if (serverQueue && serverQueue.playing) {
            ...
            return message.channel.send("⏸  **|**  gua pause nih buat lu!!!");
        }
        return message.channel.send("gada lagunya woi");

    } else if (command === "resume") {
        if (serverQueue && !serverQueue.playing) {
            serverQueue.playing = true;
            serverQueue.connection.dispatcher.resume();
            return message.channel.send("▶  **|**  gua lanjutin!!!");
        }
